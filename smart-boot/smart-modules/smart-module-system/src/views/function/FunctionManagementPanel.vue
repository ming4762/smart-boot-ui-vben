...
import type { FunctionManagementAdapter } from '../../permission-management/types';
...
const loadFunctionTreeData = async () => {
  const functions = await adapter.list({
    parameter: {
      'functionType@in': ['10', '20'],
    },
  });
  functionTreeData.value = functions.filter(
    (item: any) => item.functionType !== 'FUNCTION',
  );
};
...
<template>
  <div class="smart-table-padding h-full">
    ...
    <!--  数据权限  -->
    <DataPermissionDrawerRender v-if="adapter.dataPermission" />
  </div>
</template>
